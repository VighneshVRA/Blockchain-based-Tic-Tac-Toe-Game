...
var render = function() {
    if (typeof TicTacToe != 'undefined') {
        TicTacToe.displayBoard().then(function(res) {
            // Rendering the board based on the data fetched from the contract
            for (var i = 0; i < 9; i++) {
                var state = res[0][i].words[0];
                if (state > 0) {
                    boxes[i].className = state == 1 ? 'x' : 'o';
                    boxes[i].innerHTML = state == 1 ? 'x' : 'o';
                }
            }
        });

        checkWin(); // Checking for a win condition
        
        // Checking whose turn it is
        TicTacToe.myTurn().then(function(res) {
            if (res[0]) {
                // It's the current player's turn
                TicTacToe.winner().then(function(winner) {
                    if (winner[0] == 0) {
                        if (player === 1) {
                            gameMessages.innerHTML = "Your turn, Player 1!";
                        } else if (player === 2) {
                            gameMessages.innerHTML = "Your turn, Player 2!";
                        }
                    }
                });
            } else {
                ...
                TicTacToe.winner().then(function(winner) {
                    if (winner[0] == 0) {
                        if (player === 1) {
                            gameMessages.innerHTML = "Not your turn, Player 1!";
                        } else if (player === 2) {
                            gameMessages.innerHTML = "Not your turn, Player 2!";
                        }
                    }
                });
            }
        ...
        // Update player variable based on the turn
        TicTacToe.turn().then(function(turn) {
            player = turn[0].words[0] === 1 ? 1 : 2;
        });
    }
};
